...
class RobotAPI:
    ...
    def __init__(
        ...
        self.requester = Requester(
            base_url=self.prefix, headers=self.headers, timeout=self.timeout, logger=self.logger
        )
    ...
    def is_command_completed(self):
        ...
        if response_json.get('status', None) == 'finished':
            self.logger.info(
                f"Activity '{self.last_activity_id}' for robot '{self.robot_id}' has completed"
            )
            self.last_activity_id = None
            return True
        return False
    ...
    def localize(
        ...
        action_body = {
            ...
            'parameters': {
                'deltaPose': {'x': -pose[0], 'y': -pose[1], 'theta': -pose[2], 'frameId': map_name}
            },
        }
        response = self.requester.post_request(
    ...
    def start_activity(self, activity: str, label: str, activity_args: dict | None = None):
        ...
        self.logger.info(
            ...
            f'{activity} with response: {response_json}'
        )
